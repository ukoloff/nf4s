...
struct Node : P {
  float bulge;  ///< Measure of curvature, tan of 1/4 of arc angle; positive for
                ///< CCW arcs
...
struct Span {
  ...
  const Complex to_c() const {
    return b.to_c() - a.to_c();
  ...
  Complex linear(float f) const { return linear(Complex(f)); }
  Complex linear(Complex) const;

  bool ark() const {
    return bulge != 0;
  }  ///< Check whether span is ark (not a line)
